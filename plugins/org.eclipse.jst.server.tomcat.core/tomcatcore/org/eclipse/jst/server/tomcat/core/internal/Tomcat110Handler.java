 ...
 * Tomcat 110 handler.
 */
public class Tomcat110Handler implements ITomcatVersionHandler {
	/**
	 * @see ITomcatVersionHandler#verifyInstallPath(IPath)
	 */
	public IStatus verifyInstallPath(IPath installPath) {
		IStatus result = TomcatVersionHelper.checkCatalinaVersion(installPath, TomcatPlugin.TOMCAT_110);
		// If check was canceled, use folder check
		if (result.getSeverity() == IStatus.CANCEL) {
			result = TomcatPlugin.verifyInstallPathWithFolderCheck(installPath, TomcatPlugin.TOMCAT_110);
		}
		return result;
	}
	...
	public List getRuntimeClasspath(IPath installPath, IPath configPath) {
		...
		// 11.0 - add bootstrap.jar and tomcat-juli.jar from the Tomcat bin directory
	...
	public IStatus canAddModule(IModule module) {
		...
		if ("2.2".equals(version) || "2.3".equals(version) || "2.4".equals(version) || "2.5".equals(version)
				|| "3.0".equals(version) || "3.1".equals(version) || "4.0".equals(version) || "5.0".equals(version) || "6.0".equals(version) || "6.1".equals(version))
			return Status.OK_STATUS;
		
		return new Status(IStatus.ERROR, TomcatPlugin.PLUGIN_ID, 0, Messages.errorSpec110, null);
	}
	...
	public IStatus prepareForServingDirectly(IPath baseDir, TomcatServer server, String tomcatVersion) {
		// Nothing beyond configuration required for Tomcat 11
		return Status.OK_STATUS;
	}
	...
	/**
	 * Returns true since Tomcat 11.x supports this feature.
	 * 
	...
	/**
	 * @see ITomcatVersionHandler#supportsSecurityManager()
	 */
	@Override
	public boolean supportsSecurityManager() {
		// Tomcat 11 no longer supports running under a security manager.
		return false;
	}
